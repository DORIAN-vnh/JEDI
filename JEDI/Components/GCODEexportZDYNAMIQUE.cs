// GCode Exporter V3 : Correct circle direction + enhancements + dynamic Z
using System;
...
{
    public class GCodeExporterZ : GH_Component
    {
        public GCodeExporterZ() : base("GCode Exporter Z Dynamique", "GCodeExpZ",
        ...
        public override Guid ComponentGuid => new Guid("F209FA87-C32D-4A63-97B2-BF25D5CDE9AB");
        ...
        protected override void RegisterInputParams(GH_InputParamManager p)
        {
            ...
            p.AddCurveParameter("Z Profile", "ZP", "Optional Z profile curve along XY distance", GH_ParamAccess.item);
            p[8].Optional = true;
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            foreach (var path in paths)
            {
                ...
                double baseZ = path.Z ?? cnc?.HeightZ ?? 0;
                ...
                if (simplified == null) simplified = crv;
                if (!simplified.TryGetPolyline(out Polyline poly)) poly = new Polyline(simplified.DivideEquidistant(tol));
                ...
                double accLen = 0;
                double totalLength = zProfile != null ? zProfile.GetLength() : 0;
                ...
                for (int i = 1; i < poly.Count; i++)
                {
                    ...
                    if (zProfile != null && totalLength > 0)
                    {
                        accLen += poly[i - 1].DistanceTo(poly[i]);

                        if (zProfile.Domain.Length > 0)
                        {
                            double curveParam;
                            zProfile.LengthParameter(accLen, out curveParam);
                            z = zProfile.PointAt(curveParam).Z;
                        }
                    }

                    sb.AppendLine($"G1 X{pt.X:0.###} Y{pt.Y:0.###} Z{z:0.###} F{speed:0.#}");
                    points.Add(pt);
                }
